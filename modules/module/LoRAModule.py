...
class LoKrModule(PeftBase):
    ...
    def initialize_weights(self):
        ...
        lokr_w1_b, lokr_w1_a = self.create_layer()
        lokr_w2_b, lokr_w2_a = self.create_layer()
        self.lokr_w1_a = lokr_w1_a.weight
        self.lokr_w1_b = lokr_w1_b.weight
        self.lokr_w2_a = lokr_w2_a.weight
        self.lokr_w2_b = lokr_w2_b.weight
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # Yeah, yeah, it's different from the A/B parameters in make_weight.
        # Lycoris defines them in the opposite order. Yeah, it's confusing.
        W1 = self.make_weight(self.dropout(self.lokr_w1_b),
                              self.dropout(self.lokr_w1_a))
        W2 = self.make_weight(self.dropout(self.lokr_w2_b),
                              self.dropout(self.lokr_w2_a))
        W = (W1 * W2) * (self.alpha / self.rank)
        return self.orig_forward(x) + self.op(x, W, bias=None, **self.layer_kwargs)
